...
const BAR_COLORS = [COLORS.consultant, COLORS.professional, COLORS.other, COLORS.teacher, COLORS.legal];
...
const OBJECT_LABEL_OVERRIDES = {
  ...
  "TEACHER PAY & OTHER PROFESSIONALS": "Teacher Pay",
  "LEGAL SERVICES": "Legal Services & Lobbying",
  "LOBBYING": "Legal Services & Lobbying",
};
...
const OVERALL_OBJECT_GROUPS = [
  { key: "Building Spending", labels: ["Building Purchases"] },
  { key: "Consultant Spending", labels: ["Consultants"] },
  { key: "Teacher Pay", labels: ["Teacher Pay"] },
  { key: "Professional Services", labels: ["Professional Services"] },
];
...
export default function LongitudinalSpending() {
  ...
  const overallSeries = useMemo(() => {
    ...
    return years.map(year => {
      ...
      OVERALL_OBJECT_GROUPS.forEach(({ key, labels }) => {
        row[key] = labels.reduce((acc, label) => acc + (totalsForYear[label] || 0), 0);
      });
      return row;
    });
      ...
      <section>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={overallSeries} margin={{ top: 16, right: 24, left: 48, bottom: 16 }}>
            ...
            <Line type="monotone" dataKey="Building Spending" stroke={COLORS.building} dot={false} />
            <Line type="monotone" dataKey="Consultant Spending" stroke={COLORS.consultant} dot={false} />
            <Line type="monotone" dataKey="Teacher Pay" stroke={COLORS.teacher} dot={false} />
            <Line type="monotone" dataKey="Professional Services" stroke={COLORS.professional} dot={false} />
          </LineChart>
        </ResponsiveContainer>
