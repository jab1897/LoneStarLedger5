...
export default function LongitudinalSpending() {
  ...
  useEffect(() => {
    ...
    const loadTotals = async () => {
      try {
        const rows = await fetchCSV("/data/Spending_Longitudinal_Totals.csv");
        if (isMounted) setTotals(rows);
      } catch (error) {
        console.error("Failed to load Spending_Longitudinal_Totals.csv", error);
      } finally {
        if (isMounted) setTotalsLoaded(true);
      }
    };

    const loadObjects = async () => {
      try {
        const rows = await fetchCSV("/data/Spending_By_Object_Long.csv");
        if (isMounted) setObjects(rows);
      } catch (error) {
        console.error("Failed to load Spending_By_Object_Long.csv", error);
      } finally {
        if (isMounted) setObjectsLoaded(true);
      }
    };
  ...
  const topObjectKeys = useMemo(() => {
    if (!costCenterSeries.length) return [];
    const totalsByObject = costCenterSeries.reduce((acc, row) => {
    ...
    return Object.entries(totalsByObject)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([key]) => key);
  }, [costCenterSeries]);

  const costCenterFills = [
    COLORS.teacher,
    COLORS.nonTeacher,
    COLORS.capital,
    COLORS.other,
  ...
  return (
    <main className="px-4 md:px-8 space-y-8">
      ...
      <section>
        ...
        {!totalsLoaded && (
          <p className="text-sm text-gray-500">Loading statewide spending…</p>
        )}
        {totalsLoaded && overallSeries.length === 0 && (
          <p className="text-sm text-gray-500">Spending data is unavailable for the selected districts.</p>
        )}
      ...
      <section>
        ...
        {!objectsLoaded && (
          <p className="text-sm text-gray-500">Loading cost center details…</p>
        )}
        {objectsLoaded && topObjectKeys.length === 0 && (
          <p className="text-sm text-gray-500">Cost center data is unavailable for the selected districts.</p>
        )}
        {costCenterSeries.length > 0 && topObjectKeys.length > 0 && (
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={costCenterSeries}>
              ...
              <Legend />
              {topObjectKeys.map((key, index) => (
                <Bar key={key} dataKey={key} name={key} fill={costCenterFills[index % costCenterFills.length]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        )}
